...
import acumulados
import database
from services import alert_queue_service
...
def enfileirar_alerta_usuario(conn, usuario, telefone, mensagem, evento_id=None, prioridade=50):
    return alert_queue_service.enfileirar_alerta_usuario(
        conn, usuario, telefone, mensagem, evento_id, prioridade
    )


def enviar_alerta(mensagem, evento=None):
    log("🚨 Enfileirando alerta para usuários...")
    conn = database.get_db()
    database.garantir_tabela_alertas_fila(conn)
    database.garantir_tabela_alertas_eventos(conn)
    conn.commit()
    try:
        resultado = alert_queue_service.enfileirar_alerta(
            conn, mensagem, evento, montar_mensagem_alerta=montar_mensagem_alerta,
            enfileirar_alerta_usuario=enfileirar_alerta_usuario, atomico=False,
        )
        conn.commit()
        return resultado
    except Exception:
